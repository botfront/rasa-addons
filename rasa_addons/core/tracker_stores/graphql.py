...
class GraphqlTrackerStore(TrackerStore):
    def __init__(self, domain, url, **kwargs):
        self.front_project_id = kwargs.get('project_id')
        self.graphql_endpoint = HTTPEndpoint(url)
        self.tracker_persist_time = kwargs.get('tracker_persist_time', 3600)
        self.max_events = kwargs.get('max_events', 100)
    
        self.trackers = {}
        self.trackers_info = {}  #in this stucture we will keep the last index and the last timestamp of events in the db for a said tracker
        self.sweeper.setDaemon(True)
        self.sweeper.start()

        super(GraphqlTrackerStore, self).__init__(domain)

    def _fetch_tracker(self, sender_id, lastIndex):
        async def load():
            try:
                logger.debug('{} ->  tracker graphQL query'.format(sender_id))
                response = endpoint(GET_TRACKER, {"senderId": sender_id, "senderId": self.project_id, "after": lastIndex, "maxEvents": self.max_events})
                if "errors" in response and response["errors"]: raise urllib.error.URLError("Null response.")
                return response["data"]
            except urllib.error.URLError:
                logger.debug(f'something went wrong at {bf_url} with the query {GET_TRACKER}')
                return None
        data = await load()
    ...
    def _insert_tracker_gql(self, sender_id, tracker):
        async def insert():
            try:
                logger.debug('{} -> insert tracker graphQL mutation'.format(sender_id))
                response = endpoint(INSERT_TRACKER, {"senderId": sender_id, "projectId": self.project_id, "tracker": tracker})
                if "errors" in response and response["errors"]: raise urllib.error.URLError("Null response.")
                return response["data"]
            except urllib.error.URLError:
                logger.debug(f'something went wrong at {bf_url} with the mutation {INSERT_TRACKER}')
                return None
        data = await insert()
    ...
    def _update_tracker_gql(self, sender_id, tracker):
        async def update():
            try:
                logger.debug('{} -> update tracker graphQL mutation'.format(sender_id))
                response = endpoint(UPDATE_TRACKER, {"senderId": sender_id, "projectId": self.project_id, "tracker": tracker})
                if "errors" in response and response["errors"]: raise urllib.error.URLError("Null response.")
                return response["data"]
            except urllib.error.URLError:
                logger.debug(f'something went wrong at {bf_url} with the mutation {UPDATE_TRACKER}')
                return None
        data = await update()
        return data['updateTracker']

    def _get_last_index(sender_id)
        return self.trackers_info[sender_id].last_index

    def _get_last_timestamp(sender_id)
        return self.trackers_info[sender_id].last_timestamp

    def _store_tracker_info(sender_id,tracker_info):
        self.trackers_info[sender_id] = { "last_index": tracker_info['lastIndex'], "last_timestamp": tracker_info['lastTimestamp']}
    ...
    def retrieve(self, sender_id):
        last_index = _get_last_index(_get_last_index)
