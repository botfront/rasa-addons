...
class BotfrontTrackerStore(TrackerStore):
    ...
    def _url(self, sender_id, append):
        return "{url}/project/{project_id}/conversations/{sender_id}/{append}".format(
            url=self.front_url,
    ...
    def _get(self, sender_id):
        ...
        try:
            ...
            logger.warning(
                "Error: get request to botfront-api failed with status code {}: {}".format(
        ...
        return (
            UnsynchronizedTracker()
        )  # Return an object to represent synchronization failure with database
    ...
    def _post(self, sender_id, method, payload):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                logger.warning(
                    "Error: post request to botfront-api failed with status code {}: {}".format(
                        response.status_code, message
                    )
                )
        except Exception as e:
    ...
    def _is_none(tracker):
        return tracker is None or (
            isinstance(tracker, UnsynchronizedTracker) and not tracker.is_initialized()
        )
    ...
    def save(self, canonical_tracker):
        ...
        if self._is_none(tracker):
            ...
            if not isinstance(tracker, UnsynchronizedTracker):
                logger.debug(
                    "{} -> INSERTING TRACKER: {}".format(sender_id, serialized_tracker)
                )
        ...
        last_timestamp = (next(reversed(tracker["events"]), {})).get("timestamp", 0)
        new_events = list(
            filter(
                lambda x: x["timestamp"] > last_timestamp, serialized_tracker["events"]
            )
        )
        ...
        if not isinstance(tracker, UnsynchronizedTracker):
            ...
            logger.debug(
                "{} -> UPDATING TRACKER: {}".format(sender_id, tracker_shallow_copy)
            )
    ...
    def sweep(self):
        ...
        for key in list(self.trackers.keys()):
            ...
            if (
                ...
                and (tracker.get("latest_event_time") or float("inf"))
